...
export const useMapPage = () => {
  ...
  const [searchBoxRef, setSearchBoxRef] = useState<google.maps.places.SearchBox>(null)
  ...
  const handleChangeTheme = (changeTheme:Theme | string) => (event:React.MouseEvent<HTMLElement, MouseEvent>) => {
    setActiveTheme(changeTheme as Theme)
  }

  const onMilesChange = (miles:number) => (event:React.MouseEvent<HTMLElement, MouseEvent>) => {
