...
const MarkerComponent: React.FC<IMarkerComponent> = (
  ...
  const {marker, isActive, position, handleClickMarker, handleOnCloseClick} = props
  ...
  return (
    <ExternalMarker
      ...
      icon={{
        url: marker ? marker.active ? MARKER_STYLE[marker.typeCar] : redMarker : MARKER_STYLE['circleMarker'],
        scaledSize: new google.maps.Size(15, 15),
      }}
    >
      {(isActive && marker) && (
        <InfoWindow position={markerPosition} marker={marker} handleOnCloseClick={handleOnCloseClick} />
      )}
    </ExternalMarker>
  )
}
